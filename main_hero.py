...
class Hero(pygame.sprite.Sprite):
    ...
    def update(self, animcount,
        ...
        if flmove_down:
            ...
            if (pygame.Rect.collidelist(self.rect, [i.rect for i in room.tiles]) != -1) or pygame.Rect.collidelist(
                    self.rect, [i.rect for i in room.gates]) != -1:
                self.y -= (1 * self.speed)
                self.rect.centery -= (1 * self.speed)
    ...
    def update_weapon(self, animcount,
        ...
        for item in group:
            if (((self.rect.centerx - item.rect.centerx) ** 2 + (
                    self.rect.centery - item.rect.centery) ** 2) ** 0.5) <= self.weapon.range / 2:
                is_hit = True
                targets_hit.add(item)
        # print(targets_hit)
        if is_hit:
            range.image = pygame.transform.scale(image_range_hit, (
                image_range_hit.get_width() * (self.weapon.range / 100) + animcount // 2,
                image_range_hit.get_height() * (self.weapon.range / 100) + animcount // 2))
            self.weapon.hit(targets_hit, cur_time)
        else:
            range.image = pygame.transform.scale(image_range, (
                image_range.get_width() * (self.weapon.range / 100) + animcount // 2,
                image_range.get_height() * (self.weapon.range / 100) + animcount // 2))
        targets_hit.empty()
