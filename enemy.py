    ...
    def update(self, animcount, enemy_anim, Main_Hero, current_time, room, coins):
        ...
        if self.hp <= 0:
            ...
            if self.rect.collidepoint((self.move_x, self.move_y)):
                self.move_x = random.randint(1000 // 2 - self.room.room_w // 2 + 250,
                                             1000 // 2 + self.room.room_w // 2 - 250)
                self.move_y = random.randint(800 // 2 - self.room.room_h // 2 + 150,
                                             800 // 2 + self.room.room_h // 2 - 150)
